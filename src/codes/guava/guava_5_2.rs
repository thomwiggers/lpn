...
fn init() {
    INIT.call_once(|| {
        unsafe {
            ...
            map.insert(3, &[4]);     // 3 => [4]
            map.insert(4, &[8]);     // 4 => [8]
            map.insert(6, &[16]);     // 6 => [16]
            map.insert(5, &[9]);     // 5 => [9]
            map.insert(7, &[17]);     // 7 => [17]
...
impl BinaryCode for GuavaCode5_2 {
    fn name(&self) -> String {
        "[5, 2] Guava code".to_owned()
    }
...
mod tests {
    ...
    #[test]
    fn size() {
        let code = GuavaCode5_2.generator_matrix();
    ...
    fn random_decode_tests() {
        ...
        
        {
            ...
            let codeword = BinVector::from_bools(&[true, true, true, false, false]);
            assert_eq!(code.decode_to_code(&randvec), Ok(codeword));
        }
    ...
    fn test_generator_representation() {
        ...
        let vector = BinVector::from_bools(&[ true, false, true, true, true ]);
