...
impl BinaryCode for GuavaCode3_2 {
    fn name(&self) -> String {
        "[3, 2] Guava code".to_owned()
    }
    ...
    fn decode_to_code(&self, c: &BinVector) -> Result<BinVector, &str> {
        ...
        let mut error = BinVector::with_capacity(3);
        let stor = unsafe { error.get_storage_mut() };
        let errbytes = map[&he.as_u64()];
        debug_assert_eq!(errbytes.len(), 3 / 64 + if 3 % 64 != 0 { 1 } else { 0 });
        stor.clear();
        stor.extend_from_slice(&errbytes[..]);
        unsafe { error.set_len(3) };
    ...
    fn decode_slice(&self, c: &mut [u64]) {
        ...
        debug_assert_eq!(c[3 / 64] & !((1 << 3) - 1), 0, "this message has excess bits");
...
mod tests {
    ...
    #[test]
    fn size() {
        let code = GuavaCode3_2.generator_matrix();
    ...
    fn random_decode_tests() {
        ...
        
        {
            ...
            let codeword = BinVector::from_bools(&[false, true, true]);
            assert_eq!(code.decode_to_code(&randvec), Ok(codeword));
        }
        
        {
            ...
            let codeword = BinVector::from_bools(&[true, true, false]);
            assert_eq!(code.decode_to_code(&randvec), Ok(codeword));
        }
    ...
    fn test_generator_representation() {
        ...
        let vector = BinVector::from_bools(&[ true, false, true ]);
