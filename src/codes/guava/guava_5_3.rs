...
fn init() {
    INIT.call_once(|| {
        unsafe {
            ...
            map.insert(0, &[0]);     // 0 => [0]
            map.insert(1, &[1]);     // 1 => [1]
            map.insert(2, &[16]);     // 2 => [16]
            map.insert(3, &[17]);     // 3 => [17]
...
impl BinaryCode for GuavaCode5_3 {
    fn name(&self) -> String {
        "[5, 3] Guava code".to_owned()
    }
...
mod tests {
    ...
    #[test]
    fn size() {
        let code = GuavaCode5_3.generator_matrix();
    ...
    fn test_generator_representation() {
        ...
        let vector = BinVector::from_bools(&[ true, false, false, true, false ]);
